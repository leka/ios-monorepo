...
// MARK: - ActivityDecode_Tests

final class ActivityDecode_Tests: XCTestCase {
    func test_decodeActivityWithYAMLDecoder() throws {
    ...
    func test_decodeActivityFromYamlString() {
        if let activity = NewActivity(yaml: kTestActivityMock) {
            XCTAssertEqual(activity.id, "F8C90919AF204155A170D3957BABE7D6")
            XCTAssertEqual(activity.name, "TestActivityMock")
        }
    }
