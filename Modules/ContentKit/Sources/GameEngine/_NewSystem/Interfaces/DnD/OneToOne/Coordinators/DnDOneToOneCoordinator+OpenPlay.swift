...
// MARK: - DnDOneToOneCoordinatorOpenPlay

public class DnDOneToOneCoordinatorOpenPlay: DnDOneToOneGameplayCoordinatorProtocol {
    // MARK: Lifecycle
    ...
    public init(choices: [CoordinatorOpenPlayChoiceModel], action: NewExerciseAction? = nil, options: NewExerciseOptions? = nil) {
        ...
        self.currentOrderedChoices = Array(repeating: nil, count: choices.count)
        self.alreadyValidatedChoices = Array(repeating: nil, count: choices.count)
    }
    ...
    public func validateUserSelection() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
            self.currentOrderedChoices.enumerated().forEach { index, id in
                let choiceID = self.rawChoices.first(where: { $0.id == id })!.id
...
extension DnDOneToOneCoordinatorOpenPlay {
