...
// MARK: - OldActivityView

public struct OldActivityView: View {
    // MARK: Lifecycle

    public init(activity: OldActivity, reinforcer: Robot.Reinforcer = .rainbow) {
        self._viewModel = StateObject(wrappedValue: OldActivityViewViewModel(activity: activity))
    ...
    public var body: some View {
        ZStack(alignment: .bottomTrailing) {
            VStack {
                VStack(spacing: 15) {
                    if self.viewModel.isProgressBarVisible {
                        OldActivityProgressBar(viewModel: self.viewModel)
                    }
            ...
            if self.viewModel.isReinforcerAnimationVisible {
                ...
                if case .completed = self.viewModel.currentExerciseSharedData.state, self.viewModel.isReinforcerAnimationVisible {
                    VStack(spacing: 30) {
                        ...
                        Button(String(l10n.GameEngineKit.OldActivityView.hideReinforcerToShowAnswersButton.characters)) {
                            withAnimation {
            ...
            if case .completed = self.viewModel.currentExerciseSharedData.state, !self.viewModel.isReinforcerAnimationVisible {
                Button(String(l10n.GameEngineKit.OldActivityView.continueButton.characters)) {
                    self.continueExercise()
                }
        ...
        .alert(String(l10n.GameEngineKit.OldActivityView.QuitActivityAlert.title.characters), isPresented: self.$isAlertPresented) {
            Button(String(l10n.GameEngineKit.OldActivityView.QuitActivityAlert.cancelButtonLabel.characters), role: .cancel, action: {
                self.isAlertPresented = false
            })
            Button(String(l10n.GameEngineKit.OldActivityView.QuitActivityAlert.quitButtonLabel.characters), role: .destructive, action: {
            ...
            Text(l10n.GameEngineKit.OldActivityView.QuitActivityAlert.message)
        }
        .sheet(isPresented: self.$isInfoSheetPresented) {
            InfoDetailsView(CurationItemModel(id: self.viewModel.currentActivity.id, name: self.viewModel.currentActivity.name, contentType: .activity))
                .logEventScreenView(
    ...
    @StateObject var viewModel: OldActivityViewViewModel
...
#Preview {
    NavigationStack {
        OldActivityView(activity: OldActivity.mock)
    }
}
