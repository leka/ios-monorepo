...
// MARK: - MagicCardDetails

// swiftlint:disable identifier_name

public enum MagicCardDetails: UInt16, CaseIterable {
    case none = 0x0000
    ...
    public static func from(id: UInt16) -> MagicCardDetails? {
        allCases.first { $0.rawValue == id }
    }

    public static func from(name: String) -> MagicCardDetails? {
...
public struct MagicCard: Equatable {
    ...
    public init(_ details: MagicCardDetails, language: Language = .none) {
    ...
    public let details: MagicCardDetails
    public var language: Language

    public var id: UInt16 {
        self.details.rawValue
    }
    ...
    public static func from(id: UInt16, language: Language = .none) -> MagicCard? {
        guard let cardDetails = MagicCardDetails.from(id: id) else { return nil }
        return MagicCard(cardDetails, language: language)
    }

    public static func from(name: String, language: Language = .none) -> MagicCard? {
        guard let cardDetails = MagicCardDetails.from(name: name) else { return nil }
        return MagicCard(cardDetails, language: language)
    }
}
