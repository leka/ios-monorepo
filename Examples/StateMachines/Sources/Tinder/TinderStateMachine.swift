...
open class TinderStateMachine<State: StateMachineHashable, Event: StateMachineHashable, SideEffect> {
    ...
    public func transition(_ event: Event) throws -> Transition.Valid {
        ...
        do {
            ...
            if let action: Action = try factory?(state, event) {
                let transition: Transition.Valid = .init(
                    fromState: state,
                    event: event,
    ...
    private func notify(_ result: Transition.Result) {
        ...
        for observer in self.observers where observer.object != nil {
...
extension TinderStateMachine.Transition.Valid: Equatable
where
    State: Equatable,
    Event: Equatable,
...
extension StateMachineBuilder {
    ...
    public static func onTransition(
        _ callback: @escaping TinderStateMachine<State, Event, SideEffect>.Transition.Callback
    ) -> Component {
...
public enum StateMachineTypes {
    ...
    public struct Definition<State: StateMachineHashable, Event: StateMachineHashable, SideEffect> {
        ...
        fileprivate typealias Callbacks = [TinderStateMachine<State, Event, SideEffect>.Transition.Callback]
    ...
    public enum Component<State: StateMachineHashable, Event: StateMachineHashable, SideEffect> {
        ...
        case callback(callback: TinderStateMachine<State, Event, SideEffect>.Transition.Callback)
    }
